    ...
    if (fi.nTurbinesGlob > 0) {
      ...
      for (int iTurb = 0; iTurb < fi.nTurbinesGlob; iTurb++) {
        if (y_actuator["Turbine" + std::to_string(iTurb)]) {
          ...
          get_required(
            cur_turbine, "turbine_name", actuatorSimpleInfo->turbineName_);

          std::string turbFileName;
          get_if_present(cur_turbine, "file_to_dump_turb_pts", turbFileName);
          if (!turbFileName.empty()) {
            actuatorSimpleInfo->fileToDumpPoints_ = turbFileName;
          }

          // The correction from filtered lifting line theory
          get_if_present_no_default(cur_turbine, "fllt_correction", actuatorSimpleInfo->fllt_correction_);
          ...
          if ( epsilon_tower )
            actuatorSimpleInfo->epsilon_tower_ = epsilon_tower.as<Coordinates>();
          else
            actuatorSimpleInfo->epsilon_tower_ = actuatorSimpleInfo->epsilon_;
...
void ActuatorSimple::filtered_lifting_line()
{
  ...
  for (size_t iTurb=0; iTurb < numTurbines; iTurb++ )
  {
    ...
    auto actuatorSimpleInfo = dynamic_cast<ActuatorSimpleInfo*>(
      actuatorInfo_.at(iTurb).get());

    // If the correciton is not active for this turbine, skip this turbine
    if ( ! actuatorSimpleInfo -> fllt_correction_) continue;
      ...
      for (size_t na=0; na < ptsPerBlade; na++)
        {
          ...
          // actuator line info object of interest
          auto infoObject =
            dynamic_cast<ActuatorSimplePointInfo*>(
...
ActuatorSimple::create_actuator_point_info_map()
{
  ...
  for (size_t iTurb = 0; iTurb < actuatorInfo_.size(); ++iTurb) {
    ...
    const auto actuatorInfo =
      dynamic_cast<ActuatorSimpleInfo*>(actuatorInfo_[iTurb].get());
    ...
    if (processorId == NaluEnv::self().parallel_rank()) {
      ...
      if (!FAST.isDryRun()) {
        for (int iNode = 0; iNode < numForcePts; iNode++) {
          ...
          auto infoObject = dynamic_cast<ActuatorSimplePointInfo*>(
              actuatorPointInfoMap_.at(np).get()); 
