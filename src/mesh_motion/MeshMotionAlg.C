...
MeshMotionAlg::MeshMotionAlg(
  stk::mesh::BulkData& bulk, const YAML::Node& node, OpenfastFSI* openfast)
{
  ...
  stk::mesh::BulkData& bulk, const YAML::Node& node, OpenfastFSI* openfast)
{
  ...
  if (openfast != NULL) {
    ...
    for (auto iTurb = 0; iTurb < nTurbinesGlob; iTurb++) {
      ...
      if (fsiTurbineData != NULL) { // Could be a turbine handled through
                                    // actuator line or something
        movingFrameVec_[num_groups + n_moving_turb].reset(
          new FrameOpenFAST(bulk, node, fsiTurbineData));
        n_moving_turb += 1;
      }
    }
  }
