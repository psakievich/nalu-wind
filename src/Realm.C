...
Realm::load(const YAML::Node& node)
{
  ...
  if (y_sfm_pp) {
    surfaceFMPostProcessing_ = std::make_unique<SurfaceFMPostProcessing>(*this);
  ...
  if (node["openfast_fsi"]) {
    ...
    openfast_ = new OpenfastFSI(
      meta_data(), bulk_data(), openfastNode, surfaceFMPostProcessing_.get());
  ...
  //-------- setup_post_processing_algorithms --------------------------------
  //--------------------------------------------------------------------------
  void Realm::setup_post_processing_algorithms()
  {
  ...
  //-------- enforce_bc_on_exposed_faces  ------------------------------------
  //--------------------------------------------------------------------------
  void Realm::enforce_bc_on_exposed_faces()
  {
    ...
    stk::mesh::Selector selectRule =
      stk::mesh::Selector(*exposedBoundaryPart_) &
    ...
    if (!face_buckets.empty()) {
      ...
      for (stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
        ...
        for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
          ...
          NaluEnv::self().naluOutput()
            << " Number of elements connected to this face is: "
            << numberOfElems << std::endl;
        ...
        case CONSTANT_UD: {
          ...
          for (size_t ifield = 0; ifield < genIC.fieldNames_.size(); ++ifield) {
            ...
            stk::mesh::FieldBase* field = stk::mesh::get_field_by_name(
              genIC.fieldNames_[ifield], *metaData_);
            ThrowAssert(field);
          ...
          case VISCOSITY_ID: {
            ...
            if (isothermalFlow_) {
              ...
              if (uniformFlow_) {
                ...
                throw std::runtime_error(
                  "Realm::setup_property: Sorry, polynomial visc Ykref and "
        ...
        case IDEAL_GAS_T_P_MAT: {
          if (DENSITY_ID == thePropId) {
            ...
            if (uniformFlow_) {
              ...
              std::map<std::string, ReferencePropertyData*>::const_iterator
                itrp;
              for (itrp = materialPropertys_.referencePropertyDataMap_.begin();
                ...
                thePair =
                  std::make_pair(propData->mw_, propData->massFraction_);
              ...
              if (IDEAL_GAS_T_MAT == matData->type_) {
                ...
                rhoPropEval = new IdealGasTPropertyEvaluator(
                  pRef, universalR, mwMassFracVec);
              } else {
  ...
  //-------- extract_universal_constant --------------------------------------
  //--------------------------------------------------------------------------
  void Realm::extract_universal_constant(
  ...
  //-------- initialize_global_variables -------------------------------------
  //--------------------------------------------------------------------------
  void Realm::initialize_global_variables()
  {
    ...
    globalParameters_->set_param(
      "timeStepNm1", 1.0, needInOutput, needInRestart);
    globalParameters_->set_param(
      "timeStepCount", 1, needInOutput, needInRestart);
  ...
  //--------------------------------------------------------------------------
  void Realm::augment_property_map(
  ...
  //--------------------------------------------------------------------------
  void Realm::makeSureNodesHaveValidTopology()
  {
      ...
      if (solutionOptions_->externalMeshDeformation_) {
        ...
        for (size_t itarget = 0; itarget < targetNames.size(); ++itarget) {
          stk::mesh::Part* targetPart =
            metaData_->get_part(targetNames[itarget]);
  ...
  //-------- advance_time_step -----------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::advance_time_step()
  {
    // leave if we do not need to solve
    const int timeStepCount = get_time_step_count();
    const bool advanceMe =
      (timeStepCount % solveFrequency_) == 0 ? true : false;
    if (!advanceMe)
      return;
    NaluEnv::self().naluOutputP0()
  ...
  //--------------------------------------------------------------------------
  double Realm::compute_adaptive_time_step()
  {
  ...
  void Realm::create_mesh()
  {
    ...
    if (checkForMissingBcs_) {
      exposedBoundaryPart_ = &metaData_->declare_part(
        "exposed_boundary_part", metaData_->side_rank());
    }
  ...
  //-------- create_restart_mesh() --------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::create_restart_mesh()
  {
  ...
  void Realm::create_edges()
  {
    ...
    double start_time = NaluEnv::self().nalu_time();
    stk::mesh::create_edges(
      *bulkData_, metaData_->universal_part(), edgesPart_);
  ...
  //-------- provide_entity_count() ------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::provide_entity_count()
  {
  ...
  void Realm::delete_edges()
  {
    ...
    for (unsigned ii = 0; ii < edges.size(); ++ii) {
      ...
      if (
        bulkData_->is_valid(edges[ii]) &&
        bulkData_->bucket(edges[ii]).owned()) {
  ...
  //-------- initialize_non_conformal ----------------------------------------
  //--------------------------------------------------------------------------
  void Realm::initialize_non_conformal() { nonConformalManager_->initialize(); }

  //--------------------------------------------------------------------------
  //-------- initialize_overset ----------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::initialize_overset()
  {
  ...
  //-------- initialize_post_processing_algorithms ---------------------------
  //--------------------------------------------------------------------------
  void Realm::initialize_post_processing_algorithms()
  {
    // check for data probes
    if (NULL != dataProbePostProcessing_)
  ...
  //-------- has_mesh_motion -------------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::has_mesh_motion() const
  {
  ...
  //--------------------------------------------------------------------------
  bool Realm::has_mesh_deformation() const
  {
    if (meshMotionAlg_) {
  ...
  //-------- does_mesh_move --------------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::does_mesh_move() const
  {
  ...
  //--------------------------------------------------------------------------
  void Realm::set_current_coordinates(stk::mesh::Part * targetPart)
  {
  ...
  //-------- compute_vrtm ----------------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::compute_vrtm(const std::string& velName)
  {
    if (
      !solutionOptions_->meshMotion_ &&
      !solutionOptions_->externalMeshDeformation_)
  ...
  //-------- init_current_coordinates -----------------------------------------
  //--------------------------------------------------------------------------
  void Realm::init_current_coordinates()
  {
    const int nDim = metaData_->spatial_dimension();
  ...
  //--------------------------------------------------------------------------
  void Realm::register_symmetry_bc(
    stk::mesh::Part * part, const stk::topology& theTopo)
  {
    if (matrixFree_) {
      return;
  ...
  //--------------------------------------------------------------------------
  void Realm::register_periodic_bc(
    stk::mesh::Part * masterMeshPart, stk::mesh::Part * slaveMeshPart,
    const double& searchTolerance, const std::string& searchMethodName)
  {
  ...
  //--------------------------------------------------------------------------
  void Realm::setup_non_conformal_bc(
    stk::mesh::PartVector currentPartVec, stk::mesh::PartVector opposingPartVec,
  ...
  //--------------------------------------------------------------------------
  void Realm::register_non_conformal_bc(
    stk::mesh::Part * part, const stk::topology& theTopo)
  {
  ...
  void Realm::register_overset_bc()
  {
    for (auto* superPart : oversetBCPartVec_)
      for (auto* part : superPart->subsets()) {
        ...
        // register fields
        MasterElement* meFC =
          MasterElementRepo::get_surface_master_element(topo);
        const int numScsIp = meFC->num_integration_points();
...
#ifdef NALU_USES_TIOGA
        oversetManager_ =
          new OversetManagerTIOGA(*this, oversetBCData.userData_);
  ...
  //--------------------------------------------------------------------------
  void Realm::periodic_field_update(
  ...
  //-------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::periodic_delta_solution_update(
    stk::mesh::FieldBase * theField, const unsigned& sizeOfField,
  ...
  //--------------------------------------------------------------------------
  void Realm::periodic_max_field_update(
    stk::mesh::FieldBase * theField, const unsigned& sizeOfField) const
  {
    periodicManager_->apply_max_field(theField, sizeOfField);
  }
  ...
  void Realm::overset_field_update(
    stk::mesh::FieldBase * field, const unsigned nRows, const unsigned nCols,
    ...
    if (outputInfo_->hasOutputBlock_) {
      ...
      if (outputInfo_->userWallTimeResults_.first) {
        ...
        if (g_elapsedWallTime > outputInfo_->userWallTimeResults_.second) {
          ...
          NaluEnv::self().naluOutputP0()
            << "Realm::provide_output()::Forced Result output will be "
  ...
  //-------- provide_restart_output ------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::provide_restart_output()
  {
    stk::diag::TimeBlock mesh_output_timeblock(Simulation::outputTimer());
        ...
        for (; i != iend; ++i) {
          ...
          if (parameter.toRestartFile) {
            ioBroker_->write_global(
              restartFileIndex_, parameterName, parameter);
          }
        }
  ...
  //-------- predict_state ---------------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::predict_state() { equationSystems_.predict_state(); }

  //--------------------------------------------------------------------------
  //-------- populate_initial_condition --------------------------------------
  //--------------------------------------------------------------------------
  void Realm::populate_initial_condition()
  {
  ...
  //--------------------------------------------------------------------------
  void Realm::boundary_data_to_state_data()
  {
  ...
  //-------- populate_restart ------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::populate_restart(double& timeStepNm1, int& timeStepCount)
  {
...

      {
        for (const auto& fname : outputInfo_->restartFieldNameSet_) {
          ...
          for (unsigned i = 0; i < numStates; ++i) {
            auto* fld =
              field->field_state(static_cast<stk::mesh::FieldState>(i));
            fld->modify_on_host();
            ngp_field_manager().get_field<double>(
              fld->mesh_meta_data_ordinal());
            fld->sync_to_device();
          }
        }
      }
      ...
      if (missingFields.size() > 0) {
        ...
        if (!supportInconsistentRestart_) {
          NaluEnv::self().naluOutputP0()
            ...
            << std::endl;
          NaluEnv::self().naluOutputP0()
            << "This is applicable for a BDF2 restart run from a previously "
  ...
  //--------------------------------------------------------------------------
  double Realm::populate_variables_from_input(const double currentTime)
  {
  ...
  //--------------------------------------------------------------------------
  void Realm::populate_derived_quantities()
  {
  ...
  void Realm::set_hypre_global_id()
  {
#ifdef NALU_USES_HYPRE
    ...
    const auto& bkts =
      bulkData_->get_buckets(stk::topology::NODE_RANK, s_local);
  ...
  void Realm::check_job(bool get_node_count)
  {
    ...
    if (get_node_count) {
      ...
      if (doPromotion_) {
        if (metaData_->is_commit()) {
          ...
          nodeCount_ =
            std::pow(promotionOrder_, spatialDimension_) * nodeCount_;
          NaluEnv::self().naluOutputP0()
    ...
    if (doPromotion_) {
      ...
      unsigned edgeBWFactor =
        ...
        (promotionOrder_ + 1);
      unsigned faceBWFactor = (2 * promotionOrder_ + 1) *
                              (promotionOrder_ + 1) *
  ...
  //-------- dump_simulation_time --------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::dump_simulation_time()
  {
    NaluEnv::self().naluOutputP0() << std::endl;
    NaluEnv::self().naluOutputP0()
      << "-------------------------------- " << std::endl;
    ...
    stk::all_reduce_sum(
      NaluEnv::self().parallel_comm(), &total_time[0], &g_total_time[0],
      ntimers);
    ...
    NaluEnv::self().naluOutputP0()
      ...
      << " \tmin: " << g_min_time[0] << " \tmax: " << g_max_time[0]
      << std::endl;
    NaluEnv::self().naluOutputP0()
      << " io output fields --  "
      << " \tavg: " << g_total_time[1] / double(nprocs)
      << " \tmin: " << g_min_time[1] << " \tmax: " << g_max_time[1]
      << std::endl;
    NaluEnv::self().naluOutputP0()
      << " io populate mesh --  "
      << " \tavg: " << g_total_time[4] / double(nprocs)
      << " \tmin: " << g_min_time[4] << " \tmax: " << g_max_time[4]
      << std::endl;
    NaluEnv::self().naluOutputP0()
      << " io populate fd   --  "
      << " \tavg: " << g_total_time[5] / double(nprocs)
      << " \tmin: " << g_min_time[5] << " \tmax: " << g_max_time[5]
      << std::endl;
    NaluEnv::self().naluOutputP0()
    ...
    NaluEnv::self().naluOutputP0()
      ...
      << " \tmin: " << g_min_time[3] << " \tmax: " << g_max_time[3]
      << std::endl;
    ...
    if (realmUsesEdges_) {
      ...
      NaluEnv::self().naluOutputP0()
        << "    edge creation --  "
        << " \tavg: " << g_total_edge / double(nprocs)
    ...
    if (hasOverset_) {
      double connTime[2] = {
        oversetManager_->timerConnectivity_,
        oversetManager_->timerFieldUpdate_};
      double totTime[2], minTime[2], maxTime[2];
      stk::all_reduce_sum(
        NaluEnv::self().parallel_comm(), connTime, totTime, 2);
      stk::all_reduce_min(
        NaluEnv::self().parallel_comm(), connTime, minTime, 2);
      stk::all_reduce_max(
        NaluEnv::self().parallel_comm(), connTime, maxTime, 2);
      NaluEnv::self().naluOutputP0()
    ...
    // transfer
    if (
      hasMultiPhysicsTransfer_ || hasInitializationTransfer_ ||
      hasIoTransfer_ || hasExternalDataTransfer_) {
      ...
      NaluEnv::self().naluOutputP0()
        ...
        << " \tmin: " << g_minXfer[0] << " \tmax: " << g_maxXfer[0]
        << std::endl;
      NaluEnv::self().naluOutputP0()
        << "          execute --  "
        << " \tavg: " << g_totalXfer[1] / double(nprocs)
        << " \tmin: " << g_minXfer[1] << " \tmax: " << g_maxXfer[1]
        << std::endl;
    }
    ...
    if (doPromotion_) {
      ...
      stk::all_reduce_sum(
        NaluEnv::self().parallel_comm(), &timerPromoteMesh_, &g_totalPromote,
        1);
      ...
      NaluEnv::self().naluOutputP0()
        ...
        << " \tmin: " << g_minPromote << " \tmax: " << g_maxPromote
        << std::endl;
    }
    ...
    if (solutionOptions_->useConsolidatedSolverAlg_) {
      ...
      stk::all_reduce_sum(
        NaluEnv::self().parallel_comm(), &timerSortExposedFace_, &g_totalSort,
        1);
  ...
  //-------- provide_mean_norm -----------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::provide_mean_norm()
  {
  ...
  //-------- get_hybrid_factor -----------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_hybrid_factor(const std::string dofName)
  {
  ...
  //-------- get_alpha_factor ------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_alpha_factor(const std::string dofName)
  {
  ...
  //--------------------------------------------------------------------------
  double Realm::get_alpha_upw_factor(const std::string dofName)
  {
  ...
  //-------- get_upw_factor ------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_upw_factor(const std::string dofName)
  {
  ...
  //--------------------------------------------------------------------------
  bool Realm::primitive_uses_limiter(const std::string dofName)
  {
    bool usesIt = false;
  ...
  //-------- get_lam_schmidt -------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_lam_schmidt(const std::string dofName)
  {
  ...
  //-------- get_lam_prandtl -------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_lam_prandtl(const std::string dofName, bool& prProvided)
  {
    double factor = 1.0;
    std::map<std::string, double>::const_iterator iter =
  ...
  //-------- get_turb_schmidt ------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_turb_schmidt(const std::string dofName)
  {
  ...
  //-------- get_turb_prandtl ------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_turb_prandtl(const std::string dofName)
  {
  ...
  //-------- get_noc_usage ---------------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::get_noc_usage(const std::string dofName)
  {
  ...
  //-------- get_skew_symmetric ----------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::get_skew_symmetric(const std::string dofName)
  {
  ...
  //--------------------------------------------------------------------------
  std::string Realm::get_tanh_functional_form(const std::string dofName)
  {
  ...
  //-------- get_tanh_trans --------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_tanh_trans(const std::string dofName)
  {
  ...
  //-------- get_tanh_width --------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_tanh_width(const std::string dofName)
  {
  ...
  //-------- get_divU --------------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_divU() { return solutionOptions_->includeDivU_; }

  //--------------------------------------------------------------------------
  //-------- get_mdot_interp -------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_mdot_interp()
  {
  ...
  //--------------------------------------------------------------------------
  bool Realm::get_cvfem_shifted_mdot()
  {
  ...
  //---------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::get_cvfem_reduced_sens_poisson()
  {
  ...
  //--------------------------------------------------------------------------
  bool Realm::has_nc_gauss_labatto_quadrature()
  {
  ...
  //--------------------------------------------------------------------------
  bool Realm::get_nc_alg_upwind_advection()
  {
  ...
  //--------------------------------------------------------------------------
  bool Realm::get_nc_alg_include_pstab()
  {
  ...
  //--------------------------------------------------------------------------
  bool Realm::get_nc_alg_current_normal()
  {
  ...
  //--------------------------------------------------------------------------
  PropertyEvaluator* Realm::get_material_prop_eval(
  ...
  //-------- is_turbulent ----------------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::is_turbulent() { return solutionOptions_->isTurbulent_; }

  //--------------------------------------------------------------------------
  //-------- is_turbulent ----------------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::is_turbulent(bool isIt)
  {
    isTurbulent_ = isIt;
  ...
  //-------- needs_enthalpy --------------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::needs_enthalpy() { return needsEnthalpy_; }

  //--------------------------------------------------------------------------
  //-------- needs_enthalpy --------------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::needs_enthalpy(bool needsEnthalpy)
  {
  ...
  //-------- name ------------------------------------------------------------
  //--------------------------------------------------------------------------
  std::string Realm::name() { return name_; }

  //--------------------------------------------------------------------------
  //-------- augment_transfer_vector -----------------------------------------
  //--------------------------------------------------------------------------
  void Realm::augment_transfer_vector(
  ...
  //--------------------------------------------------------------------------
  void Realm::process_init_multi_physics_transfer()
  {
  ...
  //--------------------------------------------------------------------------
  void Realm::process_multi_physics_transfer()
  {
  ...
  //--------------------------------------------------------------------------
  void Realm::process_initialization_transfer()
  {
  ...
  //------------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::process_io_transfer()
  {
    if (!hasIoTransfer_)
      return;
  ...
  //--------------------------------------------------------------------------
  void Realm::process_external_data_transfer()
  {
  ...
  //-------- post_converged_work ---------------------------------------------
  //--------------------------------------------------------------------------
  void Realm::post_converged_work()
  {
  ...
  //-------- setup_element_promotion() ---------------------------------------
  //--------------------------------------------------------------------------
  void Realm::setup_element_promotion()
  {
  ...
  //-------- create_promoted_output_mesh -------------------------------------
  //--------------------------------------------------------------------------
  void Realm::create_promoted_output_mesh()
  {
    NaluEnv::self().naluOutputP0()
  ...
  std::vector<std::string> Realm::physics_part_names(
  ...
  //-------- get_time_step() ----------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_time_step() { return timeIntegrator_->get_time_step(); }

  double Realm::get_time_step_from_file()
  {
    return timeIntegrator_->get_time_step_from_file();
  }

  bool Realm::get_is_fixed_time_step()
  {
    return timeIntegrator_->get_is_fixed_time_step();
  }

  bool Realm::get_is_terminate_based_on_time()
  {
    return timeIntegrator_->get_is_terminate_based_on_time();
  }

  double Realm::get_total_sim_time()
  {
    return timeIntegrator_->get_total_sim_time();
  }

  int Realm::get_max_time_step_count()
  {
    return timeIntegrator_->get_max_time_step_count();
  }

  int Realm::get_restart_frequency()
  {
  ...
  //-------- get_gamma1() ----------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_gamma1() { return timeIntegrator_->get_gamma1(); }

  //--------------------------------------------------------------------------
  //-------- get_gamma2() ----------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_gamma2() { return timeIntegrator_->get_gamma2(); }

  //--------------------------------------------------------------------------
  //-------- get_gamma3() ----------------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_gamma3() { return timeIntegrator_->get_gamma3(); }

  //--------------------------------------------------------------------------
  //-------- get_time_step_count()
  //----------------------------------------------
  //--------------------------------------------------------------------------
  int Realm::get_time_step_count() const
  {
  ...
  //-------- restarted_simulation() ------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::restarted_simulation() { return outputInfo_->activateRestart_; }
  ...
  //--------------------------------------------------------------------------
  double Realm::get_stefan_boltzmann()
  {
  ...
  //--------------------------------------------------------------------------
  double Realm::get_turb_model_constant(
  ...
  //--------------------------------------------------------------------------
  TurbulenceModel Realm::get_turbulence_model() const
  {
  ...
  //-------- bulk_data() -----------------------------------------------------
  //--------------------------------------------------------------------------
  stk::mesh::BulkData& Realm::bulk_data() { return *bulkData_; }

  const stk::mesh::BulkData& Realm::bulk_data() const { return *bulkData_; }

  //--------------------------------------------------------------------------
  //-------- meta_data() -----------------------------------------------------
  //--------------------------------------------------------------------------
  stk::mesh::MetaData& Realm::meta_data() { return *metaData_; }

  const stk::mesh::MetaData& Realm::meta_data() const { return *metaData_; }

  //--------------------------------------------------------------------------
  //-------- get_activate_aura()
  //-----------------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::get_activate_aura() { return activateAura_; }
  ...
  //-------- get_tanh_blending() ---------------------------------------------
  //--------------------------------------------------------------------------
  double Realm::get_tanh_blending(const std::string dofName)
  {
  ...
  std::vector<std::string> Realm::handle_all_element_part_alias(
    const std::vector<std::string>& names) const
  {
    ...
    if (
      std::find(names.begin(), names.end(), allElementPartAlias) !=
      names.end()) {
      NaluEnv::self().naluOutputP0()
  ...
  //-------- polynomial_order() ----------------------------------------------
  //--------------------------------------------------------------------------
  int Realm::polynomial_order() const { return promotionOrder_; }

  //--------------------------------------------------------------------------
  //-------- matrix_free() ---------------------------------------------------
  //--------------------------------------------------------------------------
  bool Realm::matrix_free() const { return matrixFree_; }
